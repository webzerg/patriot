...
function* firestoreTrigger(metadata) {
  if(!metadata.trigger) return null
  yield put(firestoreDocumentFilterGetRequest({payload:metadata.trigger.payload, metadata:metadata.trigger.metadata}))
} 
...
function* documentAdd({payload, metadata}) {
  try {
    ...
    yield firestoreTrigger(metadata)
  } catch(e) {
    console.log(e)
